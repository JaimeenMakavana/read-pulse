...
export const bookStatusEnum = z.enum(['READING', 'COMPLETED', 'PAUSED', 'DROPPED', 'WISHLIST']);

export const createBookSchema = z.object({
  title: z.string().min(1, 'Title is required').max(200, 'Title must be less than 200 characters'),
  author: z.string().min(1, 'Author is required').max(200, 'Author must be less than 200 characters'),
  totalPages: z.number().int('Total pages must be an integer').min(1, 'Total pages must be greater than 0'),
